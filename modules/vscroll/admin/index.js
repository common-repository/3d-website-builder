...
wb3d_setupdata.vscroll=function(obm){
      obm.modules['vscroll']=1;

      obm.setupeffect.vscroll=function(html){
            jQuery('#vscroll_selectmenu').append(html);
      };
      obm.removeeffect.vscroll=function(eff){
            jQuery('#vscroll_selectmenu option[value="'+eff+'"]').remove();
      };
      obm.checkeffect.vscroll=function(eff,B,y){
            var flag=0;
            if(B['handle_'+y]==eff){
                  flag=1;
            }
      ...
      obm.removeobj.vscroll=function(K,idm,y){
	...
	obm.setnewvalue.vscroll=function(){
      	jQuery('.wb3d_panel').css('display','none');
      	jQuery('#vscroll_div').css('display','');
            jQuery('#vscroll_div').removeClass("wb3d_displaynone");
      	jQuery('#vscroll_upload_link1').val('');
      	jQuery('#vscroll_upload_link2').val('');
      	jQuery('#vscroll_upload_pic1').attr('src','');
      	jQuery('#vscroll_upload_pic2').attr('src','');
      	jQuery('#vscroll_overflow1').prop('checked', false);
      };

		
      obm.setvalue.vscroll=function(i,T,K){
            jQuery('.wb3d_panel').css('display','none');
            jQuery('#vscroll_div').css('display','');
            jQuery('#vscroll_div').removeClass("wb3d_displaynone");
            jQuery('#vscroll_upload_link1').val(K['img_'+i]);
            jQuery('#vscroll_upload_link2').val(K['img2_'+i]);
            jQuery('#vscroll_upload_pic1').attr('src',K['img_'+i]);
            jQuery('#vscroll_upload_pic2').attr('src',K['img2_'+i]);
            jQuery('#vscroll_selectmenu').val(K['handle_'+i]);
            if(K['overflow1_'+i]==1){
                  jQuery('#vscroll_overflow1').prop('checked', true);
            }
            else{
                  jQuery('#vscroll_overflow1').prop('checked', false);
            }
      ...
      obm.savedata.vscroll=function(zz,z,i,t,T,F,flag,cx,cy,ax,ay){
            for(var j=0;j<4;j++){
                  if(flag!=1){
                        cy=j;
                        cx=j;
            ...
            zz[z][i]['type_'+t]='vscroll';
            zz[z][i]['handle_'+t]=F['handle_'+t];
            zz[z][i]['overflow1_'+t]=F['overflow1_'+t];
      };


      jQuery('#vscroll_submit').click(function(){

            var sn=obm.sn['0-0'];
            var cu=0;
            var i,idm,K,tmpdivid;
            if(jQuery('#vscroll_selectmenu').val()==""||jQuery('#vscroll_selectmenu').val()==null){
            ...
            if(obm.new_layer!=0){
                  ...
                  K=obm.dataslide[sn][ obm.los[sn+'-'+obm.activeslide[cu]] ][obm.lo_l[sn+'-'+obm.activeslide[cu]+'-'+obm.s_l]];
                  idm=obm.wb3d_id['0_'+obm.activeslide[cu]+'-'+obm.s_l+'-'+i];
                  obm.removeobj.vscroll(K,idm,i);
                  jQuery('.wb3d_panel').css('display','none');
                  jQuery('#textmenu2').css('display','none');
            }

            wb3d_get_vscroll(K,i,obm);
            jQuery('.wb3d_panel').css('display','none');
            ...
            obm.createobj.vscroll(sn,idm,K,tmpdivid,obm,i,0,idmm,obm.activeslide[cu],cu,0);
      ...
      wb3d_vscroll_resize=function (id){


            jQuery('#wb3df-'+id+'-2' ).resizable({handles: 'e',
      ...
      function wb3d_get_vscroll(K,i,obm){
            K['type_obj-'+i  ]='vscroll';
            K['img_'+i]=jQuery('#vscroll_upload_link1').val();
            K['img2_'+i]=jQuery('#vscroll_upload_link2').val();
            if(jQuery('#vscroll_selectmenu').val()==undefined){
                  K['handle_'+i]=-1;
            }
            else{
                  K['handle_'+i]=jQuery('#vscroll_selectmenu').val();
            }     
                  
            if(jQuery('#vscroll_overflow1').is(':checked')){
                  K['overflow1_'+i  ]=1;
            }
            else{
                  K['overflow1_'+i  ]=0;
      ...
      jQuery('#vscroll_upload_link1').keyup(function(){
            jQuery('#vscroll_upload_pic1').attr('src',jQuery('#vscroll_upload_link1').val());
      });
      jQuery('#vscroll_upload_link2').keyup(function(){
            jQuery('#vscroll_upload_pic2').attr('src',jQuery('#vscroll_upload_link2').val());
      });

      jQuery('#vscroll_upload_button1').click(function(){
            ...
            frame.on( 'select', function() {
                  ...
                  jQuery('#vscroll_upload_pic1').attr('src',imgurl);
                  jQuery('#vscroll_upload_link1').val(imgurl);
      ...
      jQuery('#vscroll_upload_button2').click(function(){
            ...
            frame.on( 'select', function() {
                  ...
                  jQuery('#vscroll_upload_pic2').attr('src',imgurl);
                  jQuery('#vscroll_upload_link2').val(imgurl);
      ...
      jQuery('#vscroll_close2').click(function(){
            jQuery('.wb3d_panel').css('display','none');
      });
